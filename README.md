...
Устройство практически всегда находится в режиме глубокого сна. При кратковременном нажатии на кнопку, обнаружении протечки, обнаружении окончания протечки, а также ввод нового устройства в сеть, оно работает в штатном режиме 20 секунд. Затем оно уходит в глубокий сон на 1 час. Через час оно просыпается, поллит координатор и через 5 секунд опять засыпает на 1 час. И так по кругу. Если нужно передать какие-то настройки на устройство, его нужно сперва разбудить коротким одиночным нажатием на кнопку.
...
Прямой биндинг, это тоже не панацея. Поэтому, если уж вы все-таки построили систему автопротечки на подобных датчиках, советую  продублировать отправку команд `OnOff` на моторы кранов через автомтизацию УД.
...
- 1.0.05
	- Изменен режим глубокого сна. К сожалению самый экономный режим не позволяет командам `OnOff` уходить на устройства со 100% вероятностью.
	- Изменена структура прошивки. `Bootloader` теперь не используется.
	- Уменьшена мощность передатчика.
	- Убраны настройки репортинга. Отчет по батарейке будет приходить автоматически 1 раз каждые 4 часа. Можно отправить принудительные репорт, нажав на кнопку.
	- Увеличен период проверки обновлений до максимально возможного.
